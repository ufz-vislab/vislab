...
SpaceNavigatorClient::SpaceNavigatorClient() 
{
	...
	for(int i = 0; i < 8; i++)
		buttons[0] = false;
	_defaultButtonBehaviour = false;
	x = y = z = rx = ry = rz = 0.f;

	_dominating = false;
	_invertAxes[0] = _invertAxes[2] = _invertAxes[3] = _invertAxes[5] = 1.f;
	_invertAxes[1] = _invertAxes[4] = -1.f;
...
void SpaceNavigatorClient::mainloop()
{
...
void SpaceNavigatorClient::invertAxis(SpaceNavigatorAxes axisToInvert)
{
	...
	{
		if(_invertAxes[axisToInvert - 1] == 1.f)
			_invertAxes[axisToInvert - 1] = -1.f;
		else
			_invertAxes[axisToInvert -1] = 1.f;
	}
}

void CALLBACK SpaceNavigatorClient::_handleButtons(void *, vrpn_BUTTONCB buttonData)
{
	// read button data
...
void CALLBACK SpaceNavigatorClient::_handleAnalogs(void *, vrpn_ANALOGCB analogData)
{
	...
	if(_spacenavigator->_mode == TRANSROT)
	{
		...
		{
			float max = (float)analogData.channel[0];
			int index = 0;
			for(int i = 1; i < 6; i++)
			{
				...
				{
					index = i;
					max = (float)analogData.channel[i];
				}
			}
		...
		{
			_spacenavigator->x = (float)analogData.channel[0] * _spacenavigator->_invertAxes[0];
			_spacenavigator->y = (float)analogData.channel[2] * _spacenavigator->_invertAxes[1];
			_spacenavigator->z = (float)analogData.channel[1] * _spacenavigator->_invertAxes[2];
			_spacenavigator->rx = (float)analogData.channel[3] * _spacenavigator->_invertAxes[3];
			_spacenavigator->ry = (float)analogData.channel[5] * _spacenavigator->_invertAxes[4];
			_spacenavigator->rz = (float)analogData.channel[4] * _spacenavigator->_invertAxes[5];
		}
	}
	...
	else if(_spacenavigator->_mode == ROT)
	{
		if(_spacenavigator->_dominating)
		{
			float max = (float)analogData.channel[3];
			int index = 3;
			for(int i = 4; i < 6; i++)
			{
				if(abs(analogData.channel[i]) > abs(max))
