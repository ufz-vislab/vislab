...
int vrpn_Generic_Server_Object::setup_3DConnexion_Pilot(char * & pch, char * line, FILE * config_file) {
#ifdef VRPN_USE_HID
  char s2 [LINESIZE];

  next();
  if (sscanf(pch,"%511s",s2)!=1) {
    fprintf(stderr,"Bad 3DConnexion_Pilot line: %s\n",line);
    return -1;
  }

  // Open the 3DConnexion_Pilot
  // Make sure there's room for a new button
  if (num_buttons >= VRPN_GSO_MAX_BUTTONS) {
    fprintf(stderr,"vrpn_3DConnexion_Pilot: Too many buttons in config file");
    return -1;
  }

  // Open the button
  if (verbose) {
    printf("Opening vrpn_3DConnexion_Pilot %s\n", s2);
  }
  if ( (buttons[num_buttons] = new vrpn_3DConnexion_Pilot(s2, connection)) == NULL ) {
    fprintf(stderr,"Can't create new vrpn_3DConnexion_Pilot\n");
     return -1;
  } else {
    num_buttons++;
  }
#else
  fprintf(stderr,"vrpn_server: Can't open 3DConnexion_Pilot: HID not compiled in: define VRPN_USE_HID in vrpn_Configure.h and recompile.\n");
#endif

  return 0;  // successful completion
}

int vrpn_Generic_Server_Object::setup_3DConnexion_Explorer(char * & pch, char * line, FILE * config_file) {
#ifdef VRPN_USE_HID
  char s2 [LINESIZE];

  next();
  if (sscanf(pch,"%511s",s2)!=1) {
    fprintf(stderr,"Bad 3DConnexion_Explorer line: %s\n",line);
    return -1;
  }

  // Open the 3DConnexion_Explorer
  // Make sure there's room for a new button
  if (num_buttons >= VRPN_GSO_MAX_BUTTONS) {
    fprintf(stderr,"vrpn_3DConnexion_Explorer: Too many buttons in config file");
    return -1;
  }

  // Open the button
  if (verbose) {
    printf("Opening vrpn_3DConnexion_Explorer %s\n", s2);
  }
  if ( (buttons[num_buttons] = new vrpn_3DConnexion_Explorer(s2, connection)) == NULL ) {
    fprintf(stderr,"Can't create new vrpn_3DConnexion_Explorer\n");
     return -1;
  } else {
    num_buttons++;
  }
#else
  fprintf(stderr,"vrpn_server: Can't open 3DConnexion_Explorer: HID not compiled in: define VRPN_USE_HID in vrpn_Configure.h and recompile.\n");
#endif
  ...
  , num_imagestreams(0)
{
    ...
    {   
		...
		// Changed: 22.01.08, Bilke
	  } else if (isit("vrpn_3DConnexion_Pilot")) {
			CHECK(setup_3DConnexion_Pilot);
	  } else if (isit("vrpn_3DConnexion_Explorer")) {
			CHECK(setup_3DConnexion_Explorer);
// BUW additions
